...
public class ApkDecoder {
	...
	private String TAG=ApkDecoder.class.getSimpleName();
	...
	private void copyOtherResFiles() throws IOException {
		if (mRawResourceFiles.size() == 0) {
			Log.e(TAG, "mRawResourceFiles size for empty");
			return;
		}
		
		URI resPath = mRawResFile.toURI();
		URI destPath =mOutResFile.toURI();

		for (File path : mRawResourceFiles) {
			URI relativePath = resPath.relativize(path.toURI());
			URI dest = destPath.resolve(relativePath);
			FileOperation.copyFileUsingStream(path, new File(dest));
		}
	}
	...
	private void ensureFilePath() throws IOException {
		...
		//这里纪录原始res目录的文件
		Log.i("mRawResFile", mRawResFile.getAbsolutePath());
		addDirAllFile(mRawResFile);
		Log.i("mRawResourceFilesSize", mRawResourceFiles.size() + "");
	...
	private void addDirAllFile(File dir) {
